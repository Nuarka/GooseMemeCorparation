...
# ========= styles / systems =========
DM_SYSTEM = (
"Ты — дерзкий, тёплый гусь. Отвечай КОРОТКО (1–2 фразы, ≤25 слов). "
"Лёгкий сюр, сленг, иногда «Кря». Не морализируй. Без 18+/криминала. "
"Избегай эмодзи, канцелярщины. "
"Инструкции: Если вопрос бытовой — коротко и с оговоркой «может меняться». "
"Если болтовня — шуточный one-liner."
)

GROUP_SYSTEM = (
"Ты — гусь-наблюдатель в групповом чате. Пиши редко. Если не обращаются — иногда короткая вставка "
...
def init_db():
    # если schema.sql рядом — выполним
    if os.path.exists("schema.sql"):
        with open("schema.sql","r",encoding="utf-8") as f, closing(db()) as conn:
            conn.executescript(f.read()); conn.commit()
    else:
        with closing(db()) as conn:
            # минимально — создадим ключевые таблицы
            conn.executescript("""
...
def set_cooldown(conn, chat_id: int, key: str, seconds: int):
    conn.execute("""INSERT INTO cooldowns(chat_id,key,until_ts) VALUES (?,?,?)
                 ...
                 (chat_id, key, now()+seconds))
    conn.commit()

def cooldown_ready(conn, chat_id: int, key: str) -> bool:
    r = conn.execute("SELECT until_ts FROM cooldowns WHERE chat_id=? AND key=?",
                     (chat_id, key)).fetchone()
    return (r is None) or (r["until_ts"] <= now())

def pick_template(conn, who: str) -> Optional[str]:
    # безопасно, если колонки enabled нет
    has_enabled = any(r["name"] == "enabled"
                      for r in conn.execute("PRAGMA table_info(replies_templates)").fetchall())
    if has_enabled:
        rows = conn.execute(
            "SELECT text, weight FROM replies_templates WHERE (who=? OR who='any') AND enabled=1",
            (who,)
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT text, weight FROM replies_templates WHERE (who=? OR who='any')",
            (who,)
        ).fetchall()
    if not rows: return None
    basket = []
    for r in rows: basket += [r["text"]]*max(1, (r["weight"] or 1))
...
def local_reply(text: str) -> str:
    ...
    if random.random() < 0.35:
        base = f"{base} {random.choice(STYLE['interjections'])}."
    return base
...
def choose_persona(chat_row, chat: types.Chat) -> str:
    if chat.type == ChatType.PRIVATE:
        return chat_row["mode"] if chat_row["mode"] in ("duck","goose") else "duck"
    return "goose"
...
async def on_start(m: types.Message):
    with closing(db()) as conn:
        get_chat(conn, m.chat)
    await m.answer("Я гусь. В ЛС — отвечаю коротко, в группах — иногда встреваю. Кря.")

# --- ADMIN (скрытые) ---
@dp.message(Command("set_mode"))
async def cmd_set_mode(m: types.Message, command: Command.Object):
    if not is_admin(m.from_user.id): return await admin_only(m)
    args = (command.args or "").split()
    if len(args) != 1 or args[0] not in ("duck","goose","auto"):
        return await m.reply("usage: /set_mode duck|goose|auto")
    with closing(db()) as conn:
        conn.execute("UPDATE chats SET mode=? WHERE chat_id=?", (args[0], m.chat.id)); conn.commit()
    await m.reply(f"mode = {args[0]}")

@dp.message(Command("learning"))
async def cmd_learning(m: types.Message, command: Command.Object):
    if not is_admin(m.from_user.id): return await admin_only(m)
...
async def cmd_add_tpl(m: types.Message, command: Command.Object):
    if not is_admin(m.from_user.id): return await admin_only(m)
    # формат: /add_tpl who|any :: текст :: weight(optional)
    raw = command.args or ""
    ...
    with closing(db()) as conn:
        conn.execute("INSERT INTO replies_templates(who,text,weight,enabled) VALUES (?,?,?,1)",
                     (who, text, weight)); conn.commit()
    await m.reply("ok: template added")

@dp.message(Command("add_meme"))
async def cmd_add_meme(m: types.Message, command: Command.Object):
    if not is_admin(m.from_user.id): return await admin_only(m)
    # формат: /add_meme kind :: payload
    raw = command.args or ""
    parts = [p.strip() for p in raw.split("::")]
    if len(parts) != 2: return await m.reply("usage: /add_meme kind::payload  (kind: text|photo|sticker)")
...
# сохранять последний LLM-ответ как шаблон (если learning=on или по команде)
LAST_REPLY_CACHE = {}

@dp.message(Command("save_last"))
async def cmd_save_last(m: types.Message, command: Command.Object):
    if not is_admin(m.from_user.id): return await admin_only(m)
    ...
    with closing(db()) as conn:
        conn.execute("INSERT INTO replies_templates(who,text,weight,enabled) VALUES (?,?,?,1)", (who, text, 1))
        conn.commit()
...
async def on_message(m: types.Message):
    ...
    with closing(db()) as conn:
        ...
        persona = choose_persona(chat_row, m.chat)
        ...
        # 1) явное обращение → отвечаем (коротко, по делу)
        if addressed_to_bot():
            if cooldown_ready(conn, m.chat.id, "reply"):
                # чаще — локально, LLM только при «вопросности»
                ...
                LAST_REPLY_CACHE[m.chat.id] = reply
                await m.reply(reply)
                set_cooldown(conn, m.chat.id, "reply", 8)
                replied = True

        # 2) редко — рандомная вставка (one-liner/«Кря»)
        if not replied and cooldown_ready(conn, m.chat.id, "random"):
            if random.random() < 0.03:            # кооп «встревание»
                base = pick_template(conn, persona) or local_reply(text)
                await m.reply(decorate_short(base, force_quack=True))
                set_cooldown(conn, m.chat.id, "random", random.randint(60, 140))
                replied = True
        ...
        if cooldown_ready(conn, m.chat.id, "meme") and random.random() < 0.015:
            ...
            if mem:
                ...
                try:
                    if kind == "text":
                        await m.answer(decorate_short(payload, force_quack=True))
                    elif kind == "photo":
                        await m.answer_photo(photo=payload, caption=random.choice(["Кря.", "Мне нужен ваш хлеб."]))
                    elif kind == "sticker":
                        await m.answer_sticker(sticker=payload)
                except Exception:
                    pass
